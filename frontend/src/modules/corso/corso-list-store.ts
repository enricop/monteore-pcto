...
import corsoListExporterFields from '@/modules/corso/corso-list-exporter-fields';
...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (
          !corsoListExporterFields ||
          !corsoListExporterFields.length
        ) {
          throw new Error(
            'corsoListExporterFields is required',
          );
        }
        ...
        const response = await CorsoService.list(
          filter,
          getters.orderBy,
          null,
          null,
