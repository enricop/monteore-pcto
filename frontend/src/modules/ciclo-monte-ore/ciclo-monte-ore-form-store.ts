  ...
  actions: {
    async doInit({ commit }, id) {
      try {
        ...
        if (id) {
          record = await CicloMonteOreService.find(id);
        }
    ...
    async doCreate({ commit }, values) {
      try {
        ...
        await CicloMonteOreService.create(values);
        commit('CREATE_SUCCESS');
        Message.success(
          i18n('entities.cicloMonteOre.create.success'),
        );
        routerAsync().push('/ciclo-monte-ore');
      } catch (error) {
        Errors.handle(error);
    ...
    async doUpdate({ commit }, { id, values }) {
      try {
        ...
        await CicloMonteOreService.update(id, values);

        commit('UPDATE_SUCCESS');
        Message.success(
          i18n('entities.cicloMonteOre.update.success'),
        );
        routerAsync().push('/ciclo-monte-ore');
      } catch (error) {
        Errors.handle(error);
