...
import cicloMonteOreListExporterFields from '@/modules/ciclo-monte-ore/ciclo-monte-ore-list-exporter-fields';
...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (
          !cicloMonteOreListExporterFields ||
          !cicloMonteOreListExporterFields.length
        ) {
          throw new Error(
            'cicloMonteOreListExporterFields is required',
          );
        }
        ...
        const response = await CicloMonteOreService.list(
          filter,
          getters.orderBy,
          null,
          null,
