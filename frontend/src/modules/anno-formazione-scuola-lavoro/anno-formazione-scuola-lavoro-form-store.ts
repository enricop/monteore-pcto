  ...
  actions: {
    async doInit({ commit }, id) {
      try {
        ...
        if (id) {
          record = await AnnoFormazioneScuolaLavoroService.find(id);
        }
    ...
    async doCreate({ commit }, values) {
      try {
        ...
        await AnnoFormazioneScuolaLavoroService.create(values);
        commit('CREATE_SUCCESS');
        Message.success(
          i18n('entities.annoFormazioneScuolaLavoro.create.success'),
        );
        routerAsync().push('/anno-formazione-scuola-lavoro');
      } catch (error) {
        Errors.handle(error);
    ...
    async doUpdate({ commit }, { id, values }) {
      try {
        ...
        await AnnoFormazioneScuolaLavoroService.update(id, values);

        commit('UPDATE_SUCCESS');
        Message.success(
          i18n('entities.annoFormazioneScuolaLavoro.update.success'),
        );
        routerAsync().push('/anno-formazione-scuola-lavoro');
      } catch (error) {
        Errors.handle(error);
