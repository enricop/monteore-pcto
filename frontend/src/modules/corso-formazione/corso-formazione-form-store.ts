  ...
  actions: {
    async doInit({ commit }, id) {
      try {
        ...
        if (id) {
          record = await CorsoFormazioneService.find(id);
        }
    ...
    async doCreate({ commit }, values) {
      try {
        ...
        await CorsoFormazioneService.create(values);
        commit('CREATE_SUCCESS');
        Message.success(
          i18n('entities.corsoFormazione.create.success'),
        );
        routerAsync().push('/corso-formazione');
      } catch (error) {
        Errors.handle(error);
    ...
    async doUpdate({ commit }, { id, values }) {
      try {
        ...
        await CorsoFormazioneService.update(id, values);

        commit('UPDATE_SUCCESS');
        Message.success(
          i18n('entities.corsoFormazione.update.success'),
        );
        routerAsync().push('/corso-formazione');
      } catch (error) {
        Errors.handle(error);
